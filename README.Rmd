...
For user-facing analysis verbs, `sclet` now uses `Run*` as the canonical API naming style, such as `RunPCA()`, `RunMilo()`, and `RunCellChat()`. Legacy `run*` names are still available as compatibility aliases.
