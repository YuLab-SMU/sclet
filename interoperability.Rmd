...
## For end users
 ...
 The shortest recommended workflow is to use the standard pipeline wrapper:
...
- a logical `layer`, which is the user-facing expression view that downstream wrappers consume
...
- If legacy metadata fallback is still required for backward compatibility, keep it centralized in shared internal helpers instead of re-implementing direct `metadata(...)` reads in multiple accessors.
...
5. If the wrapper consumes expression matrices, make it resolve from `layer` first and only then to an underlying assay.
6. Expose the result through `get_*()` and `has_*()` helpers if users will need to inspect it later.
