...
+ **Basilisk Python Environment Reliability**:
  ...
  + **`GLIBCXX_... not found` — clarified host requirement.** A `GLIBCXX_3.4.31 not found` (or similar `libstdc++.so.6: version 'GLIBCXX_...' not found`) failure is a **host/toolchain** problem, not a sclet bug: a compiled Python extension was built against a newer C++ runtime than the host provides. sclet cannot upgrade a system `libstdc++` (that needs root), so it makes every basilisk run prepend the environment's own `lib`/`lib64` to `LD_LIBRARY_PATH` — this resolves the error when the environment *bundles* a newer `libstdc++` (conda-backed basilisk environments ship `libstdc++-ng`), and is a harmless no-op otherwise. Running these backends therefore requires the host to satisfy **either** of: (a) a system C++ runtime providing the required `GLIBCXX` symbol — e.g. Ubuntu ≥ 22.04 with GCC ≥ 12 (Ubuntu **20.04 and 22.04 do not** provide `GLIBCXX_3.4.31` by default), **or** (b) the conda-backed basilisk environment, which bundles its own newer `libstdc++`. A failed Python import now raises an actionable `sclet_glibcxx_not_found` error pointing to these two options instead of a raw `ImportError`.
