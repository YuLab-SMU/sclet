...
```{r grn-setup, eval = file.exists("data/allTFs_hg38.txt") && file.exists("data/motifs-v9-nr.hgnc.tbl")}
library(sclet)
sce <- pbmc
...
database_paths <- Sys.getenv("SCLET_SCENIC_DB", unset = "")
database_paths <- if (nzchar(database_paths)) database_paths else character()
...
```{r grn-run, eval = exists("scenic_db_ready") && isTRUE(scenic_db_ready)}
sce <- RunSCENIC(
  sce,
  tfs_path = tfs_path,
...
```{r grn-result, eval = exists("scenic_db_ready") && isTRUE(scenic_db_ready)}
...
```{r grn-plot, eval = exists("scenic_db_ready") && isTRUE(scenic_db_ready)}
...
```{r grn-unified, eval = exists("scenic_db_ready") && isTRUE(scenic_db_ready)}
