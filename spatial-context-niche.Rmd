...
# - RunSpatialDeconvolution requires cell2location (Python, basilisk, ~2GB env)
# - RunSpatialColocalization/RunSpatialNiche require SVP + spatial coordinate data
# - RunInSilicoPerturbation requires CellOracle (Python, basilisk)
# - A minimal SpatialExperiment would also be needed for spatial analysis
...
`RunSpatialColocalization()` now uses the SVP package as its quantitative backend. SVP performs global bivariate spatial autocorrelation analysis via `runGLOBALBV()`, computing pairwise spatial correlations between feature columns and identifying which cell types or states tend to co-occur in the same spatial regions.

When deconvolution results are already present in the object, the function auto-selects the cell type proportion columns. If you have other spatial feature columns (e.g., pathway scores), pass them explicitly through the `features` parameter.

SVP colocalization is not executed during book rendering because it requires the `SVP` package and spatial coordinates. The code below shows the intended usage pattern.

```{r spatial-coloc-svp, eval=FALSE}
library(sclet)
# sce: spatial object with deconvolution results or feature columns

# Option 1: auto-detect features from deconvolution
sce <- RunSpatialColocalization(sce, method = "SVP", name = "coloc_svp")

# Option 2: specify feature columns explicitly
sce <- RunSpatialColocalization(
    sce,
    features = c("c2l_Bcell", "c2l_Tcell", "c2l_Myeloid"),
    method = "SVP",
    name = "coloc_custom"
)

# Inspect the correlation matrix
get_colocalization(sce, element = "matrix")

# Check available colocalization records
has_colocalization(sce)
```

## Spatial Niche Detection with SVP LISA

`RunSpatialNiche()` now uses SVP's Local Indicators of Spatial Association (LISA) to detect spatial niches. `runLISA()` categorizes each spot as High-High (hotspot — the feature is high and so are its neighbors), Low-Low, High-Low, Low-High, or not significant. The High-High spots are the spatial niches where a given cell type or state is concentrated above random expectation.

Like `RunSpatialColocalization()`, features are auto-detected from prior deconvolution results when available.

```{r spatial-niche-svp, eval=FALSE}
library(sclet)
# sce: spatial object with deconvolution results or feature columns

# Auto-detect features from deconvolution
sce <- RunSpatialNiche(sce, method = "SVP", name = "niche_svp")

# Specify features explicitly
sce <- RunSpatialNiche(
    sce,
    features = c("c2l_Bcell", "c2l_Tcell"),
    method = "SVP",
    name = "niche_custom"
)

# LISA categories are stored in colData with 'lisa_' prefix
head(SingleCellExperiment::colData(sce)[, grep("^lisa_", colnames(colData(sce)))])
