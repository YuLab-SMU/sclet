...
3. `Symphony` as a harmonized atlas projection backend now fully integrated
...
# Check mapping state
get_mapping(object, id = "symphony_demo")
```

Symphony mapping also works through `RunReferenceWorkflow()`:

```{r symphony-workflow, eval=FALSE}
object <- RunReferenceWorkflow(
    object,
    ref = ref,
    labels = "cell_type",
    method = "Symphony",
...
```{r run-scvi, eval=FALSE}
# Integration using scVI (cached in bookdown/data/rm_scvi_pbmc.rds)
