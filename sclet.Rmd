...
# Ensure gene symbols are used as rownames for compatibility with SingleR
if (any(grepl("^ENSG", rownames(pbmc)))) {
  library(scuttle)
  library(SummarizedExperiment)
  # Try to find symbol column
  rd <- rowData(pbmc)
  sym_col <- grep("Symbol", colnames(rd), value=TRUE)
  if (length(sym_col) > 0) {
    rownames(pbmc) <- scuttle::uniquifyFeatureNames(rownames(pbmc), rd[[sym_col[1]]])
  }
}
```
...
## Cell cluster annotation {#manual-annotation}
 ...
 ```{r umap-pbmc-cell}
 new_labels <- c("CD8+ T", "B", "Memory CD4+", "CD14+ Mono", "NK", "FCGR3A+ Mono", "Naive CD4+ T", "DC", "Platelet")
