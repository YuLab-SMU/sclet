...
# NOTE: Velocity chunks now run in CI using scRNAseq::HermannSpermatogenesisData().
# CellRank and downstream fate chunks use eval=FALSE because they:
# - require CellRank via Python basilisk (~1GB env)  
# - depend on velocity output (computationally heavy for CI)
...
```{r velocity-hvgs, eval = eval_chunks && requireNamespace("scRNAseq", quietly = TRUE)}
library(sclet)
library(scuttle)
library(scran)
