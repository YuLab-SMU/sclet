...
1. *"I have spliced/unspliced assays — how do I get all the way to fate probabilities?"* Start with the [trajectory mainline workflow](#trajectory-mainline-workflow), pass through [RNA velocity](#run-velocity-analysis), then let [CellRank](#cellrank) convert direction into terminal states and fate probabilities. The [`RunTrajectoryWorkflow()`](#workflow-shell-for-velocity-and-fate) section shows the whole chain as one call.
2. *"I already have RegVelo results on a GPU server — how do they connect to CellRank?"* Jump to [wiring RegVelo into CellRank](#regvelo-cellrank).
3. *"How do I tell whether the result is real or a smoke screen?"* Read [diagnosing the velocity-fate chain](#velocity-diagnostics). This section makes the diagnostic helpers (`CellRankSummary()`, `VelocityFateCorrelation()`, and their plots) part of the main narrative instead of an afterthought.

All runs along the way write into the same state layer described in [state-aware analysis](#state-aware), so every intermediate result can be inspected by id and replayed without recomputation.
...
## Trajectory mainline workflow {#trajectory-mainline-workflow}
...
### Run velocity analysis {#run-velocity-analysis}
...
## Already have RegVelo results? Wiring them into CellRank {#regvelo-cellrank}
...
## Workflow shell for velocity and fate {#workflow-shell-for-velocity-and-fate}
...
## Is it real or a smoke screen? Diagnosing the velocity-fate chain {#velocity-diagnostics}
...
### Reading the CellRank summary

Three things in the `CellRankSummary()` output deserve attention before you interpret anything else:

1. **Number of terminal states.** One terminal state on a development-style dataset usually means the transition matrix collapsed — either the velocity field is too weak, or the neighborhood graph is dominated by one cluster. Treat it as a workflow smoke check, not biology.
2. **Size balance of terminal states** (`terminal_states` table). A "terminal" state holding a handful of cells is more likely an artifact of the kNN graph than a genuine sink.
3. **Shape of the fate probabilities** (`fate_probabilities` table). Compare min/median/max per fate column: if max - median is near zero, fate probability carries no cell-to-cell information and downstream gene drivers inherit that flatness.
...
### Does velocity actually explain fate?
...
### Auditing the whole chain in one place
...
```{r velocity-diag-audit, eval=TRUE}
tail(CommandLog(regvelo_sce), 4)

c(
  velocity = has_velocity(regvelo_sce),
  trajectory = has_trajectory(regvelo_sce)
)
```
