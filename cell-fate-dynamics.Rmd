...
```{r velocity-data, eval = velocity_cache_ready || dynamics_cache_ready}
sce <- if (dynamics_cache_ready) {
  readRDS(dynamics_cache)
} else {
  readRDS(velocity_cache)
}
sce
```

```{r velocity-hvgs, eval = velocity_cache_ready || dynamics_cache_ready}
...
```{r velocity, eval = dynamics_cache_ready}
get_velocity(sce)
```
...
```{r velocity-plot, eval = dynamics_cache_ready && requireNamespace("velociraptor", quietly = TRUE)}
...
```{r cellrank-run, eval = dynamics_cache_ready}
